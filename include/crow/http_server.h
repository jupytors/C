    ...
    public:
      Server(Handler* handler,
        ...
        void run()
        {
            ...
            asio::ip::address bindaddr_  =  acceptor_.local_endpoint().address();
            std::string ip_str = bindaddr_.to_string();
            ip_str = ip_str == "0.0.0.0" ? "127.0.0.1" : ip_str ;
            CROW_LOG_INFO << server_name_
                          << " server is running at " << (handler_->ssl_used() ? "https://" : "http://")
                          << ip_str << ":" << acceptor_.local_endpoint().port() << " using " << concurrency_ << " threads";
